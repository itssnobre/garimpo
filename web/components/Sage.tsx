...
export default function Sage({ loteId, loteNome }: { loteId?: string; loteNome?: string }) {
  ...
  const render = (c: string) => c.split(/(\/app\/imovel\/[^\s)]+)/g).map((p, k) => p.startsWith("/app/imovel/") ? <Link key={k} href={p} style={{ textDecoration: "underline", color: "var(--accent-ink)" }}>abrir lote</Link> : p);
  return (
    <div className="chat">
