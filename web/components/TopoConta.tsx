...
export default function TopoConta() {
  ...
  if (estado === "logado") return <Link href="/app/buscar" className="btn ouro topo-cta">{t("Abrir a plataforma")}</Link>;
  return (
    <span className="topo-conta" style={{ visibility: estado === "checando" ? "hidden" : "visible" }}>
      <Link href="/entrar" className="btn sec topo-cta">{t("Entrar")}</Link>
      <Link href="/entrar?modo=criar" className="btn ouro topo-cta">{t("Criar conta")}</Link>
    </span>
  );
}
