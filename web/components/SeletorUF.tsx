  ...
  return (
    <div className="seluf">
      <button className={`fbtn ${ufs.length ? "on" : ""}`} onClick={() => setAberto((a) => !a)} aria-expanded={aberto}>
        <Pino /><span>{ufs.length === 0 ? t("Todo o Brasil") : ufs.length <= 3 ? ufs.join(" · ") : t("{n} estados", { n: ufs.length })}</span><i className="conta">{total.toLocaleString(loc)}</i>
      </button>
      {aberto && (<>
        <div className="fpanel-fundo" onClick={() => setAberto(false)} />
        <div className="fpanel seluf-panel" role="dialog" aria-label={t("Estados")}>
          <div className="fpanel-cab"><b>{t("Onde você compra")}</b><button className="btn ghost mini" onClick={() => setAberto(false)} aria-label={t("Fechar")}><Xis /></button></div>
          <p className="seluf-dica">{t("Escolha um ou mais estados. Os números são os lotes disponíveis hoje (leilão aberto, sem veto, sem valor suspeito). Sem escolha, carrega o país inteiro ({n} lotes, mais pesado).", { n: DISPONIVEIS_TOTAL.toLocaleString(loc) })}</p>
          <div className="seluf-grade">
            <button className={`ufchip ${ufs.length === 0 ? "on" : ""}`} onClick={() => onChange([])}>{t("Brasil")}<small>{DISPONIVEIS_TOTAL.toLocaleString(loc)}</small></button>
            {UFS_DISPONIVEIS.map((u) => <button key={u} className={`ufchip ${ufs.includes(u) ? "on" : ""}`} onClick={() => alternar(u)} title={t(UFS_NOMES[u])}>{u}<small>{disponiveisUF(u).toLocaleString(loc)}</small></button>)}
          </div>
        </div>
      </>)}
    </div>
