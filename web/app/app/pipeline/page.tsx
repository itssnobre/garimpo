        ...
        <div key={et} className="coluna"><h3>{et}<b>{col.length}</b></h3>
          {col.map((i) => { const a = ativo ? avaliarPadrao(i, ativo) : null; return (
            <div key={i.id} className="kcard">
              ...
              <div className="s">{i.cidade} · {brl(i.lance_minimo)}{a ? ` · máx 30% ${brl(a.res.lanceMax30)} · score ${a.score}` : ""}</div>
...
export default function Pipeline() { return <Portao titulo="Pipeline"><Conteudo /></Portao>; }
